vim.api.nvim_create_autocmd({ "BufEnter", "BufNew" }, {
	...
	callback = function()
		...
		vim.api.nvim_create_user_command(
			...
			-- Conda
			[[call system('source ~/bin/miniconda3/bin/activate jupynium && python -m jupynium --nvim_listen_addr ' . v:servername . ' &')]],
			{}
		)
...
vim.api.nvim_create_autocmd({ "BufEnter", "BufNew" }, {
  pattern = { "*.ju.py" },
  callback = function()
    vim.keymap.set({ "n", "x", "o" }, "[j", "<cmd>lua require'jupynium-textobjects'.goto_previous_cell_separator()<cr>")
    vim.keymap.set({ "n", "x", "o" }, "]j", "<cmd>lua require'jupynium-textobjects'.goto_next_cell_separator()<cr>")
    vim.keymap.set({ "n", "x", "o" }, "<space>c", "<cmd>lua require'jupynium-textobjects'.goto_current_cell_separator()<cr>")
