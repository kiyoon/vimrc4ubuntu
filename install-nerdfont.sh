...
# nerd fonts
if [ ! command -v fc-list &> /dev/null ] || ! fc-list | grep -q "Nerd Font"
then
	echo "Nerd Font could not be found. Installing UbuntuMono NF on $FONTDIR"
	mkdir -p "$FONTDIR"
	...
	| cut -d : -f 2,3 \
	| tr -d \" \
	| wget -qi - -O "$TEMPDIR/ubuntumononf.zip"
	unzip "$TEMPDIR/ubuntumononf.zip" -d $TEMPDIR/nerd-fonts
